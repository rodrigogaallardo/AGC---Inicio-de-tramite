                ...
                using (TextWriter tw = new StreamWriter(fs))
                {
                    foreach (var item in listSolicitudes)
                    {
                        try
                        {
                            ...
                            ReportingEntity Report = service.GetPDFSolicitud(item.IdSolicitud, true);
                            azBL.GuardarPDFSolicitud(item.IdSolicitud, Report.Id_file, Report.FileName, item.CreateUser);
                            ContProcesados += 1;
