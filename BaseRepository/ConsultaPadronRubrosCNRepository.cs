...
{
    public class ConsultaPadronRubrosCNRepository : BaseRepository<CPadron_RubrosCN>
    {
        private readonly IUnitOfWork _unitOfWork;

        public ConsultaPadronRubrosCNRepository(IUnitOfWork unit) : base(unit)
        {
            if (unit == null)
        ...
        /// <returns></returns>	
        public IEnumerable<CPadron_RubrosCN> GetByFKIdConsultaPadron(int IdConsultaPadron)
        {
            IEnumerable<CPadron_RubrosCN> domains = _unitOfWork.Db.CPadron_RubrosCN.Where(x =>
        ...
        public IEnumerable<ConsultaPadronRubrosEntity> GetRubros(int IdConsultaPadron)
        {
            var resolution =
                            ...
                            join crub in _unitOfWork.Db.CPadron_RubrosCN on sol.id_cpadron equals crub.id_cpadron
                            ...
                            where sol.id_cpadron == IdConsultaPadron
                            select new ConsultaPadronRubrosEntity()
                            {
                                IdConsultaPadronRubro = crub.id_rubro,
