...
namespace SSIT.Solicitud.Habilitacion.Controls
{
    ...
    public partial class BuscarUbicacion : System.Web.UI.UserControl
    {
        ...
        private static IEnumerable<ItemDirectionDTO> resultados = new List<ItemDirectionDTO>();
        ...
        SSITSolicitudesUbicacionesPropiedadHorizontalBL sSITSolicitudesUbicacionesPropHorBL = null;
        List<SSITSolicitudesUbicacionesPropiedadHorizontalDTO> sSITSolicitudesUbicacionesPropiedadHorizontalDTO = null;
        SSITSolicitudesUbicacionesPuertasBL sSITSolicitudesUbicacionesPuertasBL = null;
        List<SSITSolicitudesUbicacionesPuertasDTO> lstSSITSolUbicPuertasDTO = null;
        SSITSolicitudesUbicacionesBL solUbicBL = null;
        SSITSolicitudesUbicacionesDTO solUbicDTO = null;
        ...
        public void editar()
        {
            ...
            {
                int IdSolicitudUbicacion = this.idUbicacion;

                _editar = true;


                SSITSolicitudesUbicacionesBL solUbicacionesBL = new SSITSolicitudesUbicacionesBL();
                var dto = solUbicacionesBL.Single(IdSolicitudUbicacion);
        ...
        protected void btnEditarUbicacion_Click(object sender, EventArgs e)
        {
            ...
            SSITSolicitudesUbicacionesBL ssitSolUbicBL = new SSITSolicitudesUbicacionesBL();

            int idubicacion = (int)gridubicacion.DataKeys[gridubicacion.Rows.Count - 1].Value;
            var result = ssitSolUbicBL.GetByFKIdSolicitud(IdSolicitud).Where(x => x.IdUbicacion == idubicacion).FirstOrDefault();
        ...
        private void BuscarUbicaciones(int nrotab)
        {
            ...
            pnlValidacionIngresoUbicacion.Style["display"] = "none";
            UbicacionesBL ubicacionesBL = new UbicacionesBL();
            SSITSolicitudesUbicacionesBL SsitUbicacionesBL = new SSITSolicitudesUbicacionesBL();
            ...
            if (result.Count == 0)
            {
                ...
                lblMsjAtencion.Text = "La ubicaci&oacute;n ingresada para su consulta no existe catrastalmente.";
            }
            else
            {
                lblMsjAtencion.Text = "La puerta ingresada para su consulta no existe catrastalmente Por favor, seleccione una puerta de la parcela que corresponde, en las opciones que se encuentran dentro de los resultados de busqueda.";
                if (result.Count > 1)
                {
        ...
        protected void gridubicacion_OnRowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (_editar && this.idUbicacion != 0)
                {
                    ...
                    if (solUbicBL == null)
                        solUbicBL = new SSITSolicitudesUbicacionesBL();

                    if (solUbicDTO == null)
                        solUbicDTO = solUbicBL.Single(idUbicacion);

                    string Otros = solUbicDTO.DeptoLocalUbicacion ?? "";
                    string Local = solUbicDTO.Local ?? "";
                    string Depto = solUbicDTO.Depto ?? "";
                    string Torre = solUbicDTO.Torre ?? "";
        ...
        {
            if ((seccion.HasValue && dir != null) && (dir.Numero != null))
            {
        ...
        protected void lstPuertas_ItemDataBound(object sender, DataListItemEventArgs e)
        {
            ...
            if (Edicion)
            {
                if (sSITSolicitudesUbicacionesPuertasBL == null)
                    sSITSolicitudesUbicacionesPuertasBL = new SSITSolicitudesUbicacionesPuertasBL();

                if (lstSSITSolUbicPuertasDTO == null)
                    lstSSITSolUbicPuertasDTO = sSITSolicitudesUbicacionesPuertasBL.GetByFKIdSolicitudUbicacion(idUbicacion).ToList();
                ...
                if (lstSSITSolUbicPuertasDTO.Select(p => p.CodigoCalle).Contains(Codigo) && lstSSITSolUbicPuertasDTO.Select(p => p.NroPuerta).Contains(Numero))
        ...
        protected void CheckBoxListPHorizontales_DataBound(object sender, EventArgs e)
        {
            if (Edicion)
            {
                if (sSITSolicitudesUbicacionesPropHorBL == null)
                    sSITSolicitudesUbicacionesPropHorBL = new SSITSolicitudesUbicacionesPropiedadHorizontalBL();

                if (sSITSolicitudesUbicacionesPropiedadHorizontalDTO == null)
                    sSITSolicitudesUbicacionesPropiedadHorizontalDTO = sSITSolicitudesUbicacionesPropHorBL.GetByFKIdSolicitudUbicacion(idUbicacion).ToList();

                CheckBoxList chk = (CheckBoxList)sender;

                foreach (ListItem item in chk.Items)
                {
                    if (sSITSolicitudesUbicacionesPropiedadHorizontalDTO.Select(p => p.IdPropiedadHorizontal).Contains(Convert.ToInt32(item.Value)))
                        item.Selected = true;
                }
            }
