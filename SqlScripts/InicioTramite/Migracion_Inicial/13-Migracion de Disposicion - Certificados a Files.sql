...
SELECT @id_tipdocsis =  id_tipdocsis FROM TiposDeDocumentosSistema WHERE cod_tipodocsis = 'DISPOSICION_HABILITACION'
SET @cantReg = 0
...
FROM
	Certificados
WHERE
	 TipoTramite = 6		-- Disposición de Habilitación
ORDER BY 
	NroTramite
...
BEGIN
	...
	SET @FileName = 'Disposicion-' + CAST(@id_file as nvarchar) + '.' + @formato_archivo
